...
def index():
    ...
    return render_template('index.html', inventory=inventory.to_dict(orient='records'), 
                           ...
                           products=[item["Product"] for item in inventory.to_dict(orient='records')])
...
def add_entry():
    ...
    if file_type == 'inventory':
        ...
        order_dict = {"Order ID": new_order_id}
        for i, (product, quantity) in enumerate(zip(products, quantities)):
            if i == 0:
                order_dict["Product"] = product
                order_dict["Qty"] = quantity
            else:
                order_dict[f"Product.{i}"] = product
                order_dict[f"Qty.{i}"] = quantity
